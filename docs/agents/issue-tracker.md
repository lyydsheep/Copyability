...
- **Blocking**: use GitHub's native issue dependencies. Where dependencies aren't available, fall back to a `Blocked by: #<n>, #<n>` line at the top of the child body.
- **Frontier query**: list the map's open children, then exclude tickets with an open blocker or assignee; first in map order wins.
